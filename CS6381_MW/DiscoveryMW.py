...
class DiscoveryMW():
    ...
    def handle_reply(self):
        try:
            ...
            if (disc_resp.msg_type == discovery_pb2.TYPE_REGISTER):
                ...
                timeout = self.register_reply(disc_resp.register_resp.status)
            if (disc_resp.msg_type == discovery_pb2.TYPE_ISREADY):
                #make an upcall to the application logic
    ...
    def handle_request(self):
        try:
            ...
            if (disc_req.msg_type == discovery_pb2.TYPE_REGISTER):
                ...
                tieout = self.upcall_obj.isready_loop(disc_req.isready_req_dht)
    ...
    def propagateRegister(self, next, reg_req_dht, end, dest):
        self.logger.info("DiscoveryMW::propagateRegister: building dht request")
        disc_req = discovery_pb2.DiscoveryReq()
        disc_req.msg_type = discovery_pb2.TYPE_REGISTER_DHT
        disc_req.register_req_dht.role = reg_req_dht.role
        disc_req.register_req_dht.info.CopyFrom(reg_req_dht.info)
        disc_req.register_req_dht.topiclist[:] = reg_req_dht.topiclist
        disc_req.register_req_dht.end = end
        disc_req.register_req_dht.dest = dest
    ...
    def propagateIsReady(self, next, count_pub, count_sub, origin):
        ...
        disc_req.isready_req_dht.count_pub = count_pub
        disc_req.isready_req_dht.count_sub = count_sub
        disc_req.isready_req_dht.origin = origin
